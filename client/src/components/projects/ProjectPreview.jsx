...
export function ProjectPreview({ project, isCardHovered = false, isLaptopHovered = false }) {
  ...
  // Show "View Live Project" overlay only when mouse is directly over the laptop / screen
  const showLiveButton = isLaptopHovered || internalHover;
  ...
  return (
    ...
    >
      {/* ================= 1. STANDBY MODE (pahle ase band ho) ================= */}
      {!isCardHovered && (
        <div className="relative w-full h-full bg-[#030610] flex flex-col items-center justify-center cursor-pointer transition-all duration-300">
      ...
      {/* ================= 2. ACTIVE SCREEN (jab cart par hubber ho tab hi img ay) ================= */}
      {isCardHovered && (
        <div className="relative w-full h-full flex flex-col bg-slate-950 animate-fadeIn">
          ...
          <div className="relative flex-1 w-full overflow-hidden bg-slate-950">
            ...
            {/* ================= 3. VIEW LIVE PROJECT (or jab laptop par hubber ho tab view live projecct ka ay) ================= */}
            <div
